...
const fn finalize(mut input: u32, data: &[u8], mut cursor: usize) -> u32 {
    ...
    while len >= 4 {
        input = input.wrapping_add(get_32bits(data, cursor).wrapping_mul(PRIME32_3));
        cursor += mem::size_of::<u32>();
        len -= mem::size_of::<u32>();
        input = input.rotate_left(17).wrapping_mul(PRIME32_4);
    }

    while len > 0 {
        input = input.wrapping_add((data[cursor] as u32).wrapping_mul(PRIME32_5));
        cursor += mem::size_of::<u8>();
        len -= mem::size_of::<u8>();
        input = input.rotate_left(11).wrapping_mul(PRIME32_1);
    }
